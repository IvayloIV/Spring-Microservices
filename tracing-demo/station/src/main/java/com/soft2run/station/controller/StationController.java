...
public class StationController {
    ...
    @Autowired
    public StationController(WebClient webClient) {
    ...
    public Station getStation(@PathVariable Long id) {
        String url = UriComponentsBuilder.fromUriString("http://localhost:8081/vehicle/{id}").buildAndExpand(id).toUriString();

        String vehicle = webClient.get()
                .uri(url)
                .retrieve()
